...
def HVS_with_device(number_LS, request_date):
    from prototype import indications_HVS
...
def GVS_heat_carrier_with_device(number_LS, request_date):
    #вытаскивание прошлых данных
    try:
        cursor.execute("""SELECT * FROM Customers WHERE number_ls='%s' ORDER BY indications_date DESC LIMIT 1 OFFSET 1;""",(number_LS,)) 
        datas=cursor.fetchall()
    ...
    from prototype import indications_GVS
...
def EE_with_device(number_LS, request_date):
    from prototype import indications_EE_daytime, indications_EE_night
...
def activate_calculations():
    from prototype import number_of_residents, HVS_device, number_LS, request_date, GVS_device, EE_device
